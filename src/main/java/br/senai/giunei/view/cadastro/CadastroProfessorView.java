...
public class CadastroProfessorView extends JPanel {
	...
	public CadastroProfessorView(String titulo, Usuario logado) {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (professor.getId() != null) {
					...
					if (!senha.equals("") || !senha.isEmpty()) {
						professor.setSenha(Criptografia.md5(senha));
					}
					new UsuarioDAO().atualizar(professor);
					JOptionPane.showMessageDialog(null, "Professor atualizado");
				} else {
					// salvar
					professor.setSenha(Criptografia.md5(senha));
					new UsuarioDAO().salvar(professor);
		...
		JLabel lblNewLabel_2_1 = new JLabel("Lista de Professores");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (table.getSelectedRow() >= 0) {
					...
					if (escolha == 0) {
						professor = professores.get(table.getSelectedRow());
						new UsuarioDAO().deletar(professor);
						carregarTabela();
					}
		...
		btnEditar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (table.getSelectedRow() >= 0) {
					professor = professores.get(table.getSelectedRow());
					idTF.setText(professor.getId().toString());
					nomeTF.setText(professor.getNome().toString());
					emailTF.setText(professor.getEmail().toString());
					generoCB.setSelectedIndex(professor.getGenero().ordinal());
		...
		btnMostrar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (table.getSelectedRow() >= 0) {
					...
					DetalheView dialog = new DetalheView(professor.toString(), "Detalhe do Usuário");
	...
	private void carregarTabela() {
		...
		if (!professores.isEmpty()) {
			...
			for (int i = 0; i < professores.size(); i++) {
				...
				dados[i][1] = professores.get(i).getEmail();
				dados[i][2] = professores.get(i).getGenero().getStr();
				dados[i][3] = professores.get(i).getPerfil().getStr();
			}
		}
